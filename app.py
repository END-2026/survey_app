    ...
    with st.spinner("Загрузка данных..."):
        try:
            ...
            if not data:
                ...
                st.write("Скачайте собранные данные в удобном формате:")
                
                # Подготовка данных для экспорта (с русскими заголовками)
                export_df = df_ru.copy()
                
                # Преобразуем список платформ в строку для CSV/Excel
                export_df["Платформы"] = export_df["Платформы"].apply(lambda x: ", ".join(x) if isinstance(x, list) else x)
                
                # 🔧 ИСПРАВЛЕНИЕ: Убираем информацию о часовом поясе из даты
                if "Время отправки" in export_df.columns:
                    # Преобразуем в строку без часового пояса
                    export_df["Время отправки"] = export_df["Время отправки"].dt.strftime("%Y-%m-%d %H:%M:%S")
                ...
                with col_export1:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"опрос_анонимные_отзывы_{current_date}.csv",
                        mime="text/csv",
                        use_container_width=True,
                        help="Скачать данные в формате CSV (совместимо с Excel)"
                    )
                ...
                with col_export2:
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        export_df.to_excel(writer, index=False, sheet_name='Ответы')
                    excel_data = output.getvalue()
                    
                    st.download_button(
                        label="📊 Скачать в формате Excel",
                        data=excel_data,
                        file_name=f"опрос_анонимные_отзывы_{current_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
                        help="Скачать данные в формате Excel (.xlsx)"
                    )
